...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "zapier-pp-cli", "config.json"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "zapier-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("zapier", "test")
	RegisterTools(s)
