...
type Config struct {
	...
	ZapierSessionCookie string          `json:"session_cookie"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("ZAPIER_SESSION_COOKIE"); v != "" {
		cfg.ZapierSessionCookie = v
		cfg.markEnvOverride("ZapierSessionCookie")
		cfg.AuthSource = "env:ZAPIER_SESSION_COOKIE"
		cfg.CredentialSource = "env:ZAPIER_SESSION_COOKIE"
	}
	// Label config-file-derived credentials so doctor can distinguish
	// "credentials persisted on disk" from "no credentials at all" — without
	// this, users who saved via the local browser capture without an env var see a blank
	...
	if v := os.Getenv("ZAPIER_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("ZAPIER_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "zapier-pp-cli", "config.json"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.ZapierSessionCookie
	if token == "" {
		return ""
	}
	if c.ZapierSessionCookie == "" {
		return ""
	}
	replacements := map[string]string{
		"token":                 token,
		"session_cookie":        c.ZapierSessionCookie,
		"ZAPIER_SESSION_COOKIE": c.ZapierSessionCookie,
	}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.ZapierSessionCookie != "" {
		parts = append(parts, "session_cookie="+c.ZapierSessionCookie)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		ZapierSessionCookie: c.ZapierSessionCookie,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.ZapierSessionCookie == "" {
		c.ZapierSessionCookie = creds.ZapierSessionCookie
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.ZapierSessionCookie = token
	delete(c.envOverrides, "ZapierSessionCookie")
	c.updateFileConfigField("ZapierSessionCookie")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ZapierSessionCookie"] {
			out.ZapierSessionCookie = c.fileConfig.ZapierSessionCookie
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ZapierSessionCookie":
		c.fileConfig.ZapierSessionCookie = c.ZapierSessionCookie
	}
}
